...
#======================================================================================================#
#===========================================================
            elif 'imgsearch:' in msg.text.lower():
            	try:
            	query = msg.text.lower().replace('imgsearch:', '')
                query = query.replace(' ', '+')
                path = 'https://cryptic-ridge-9197.herokuapp.com/api/imagesearch/%s?offset=1' % query
                r = requests.get(path)
                data = r.json()
                result = 'Result of %s' % query
                if snippet==[]:
                   pass
               else:
                   for a in range(int(len(snippet))):
                       del snippet[int(a)]
               if url==[]:
                   pass
               else:
                   for b in range(int(len(url))):
                       del url[int(b)]
               if context==[]:
                   pass
               else:
                   for c in range(int(len(context))):
                       del context[int(c)]
               for i,data in enumerate(data,1):
                   url.append(data['url'])
                   snippet.append(data['snippet'])
...
#======================================================================================================#
               ...
               no = msg.text.replace("Pranksms ","")
               r = requests.get("http://apisora2.herokuapp.com/prank/sms2/?no="+str(no))
               data = r.json()
               result = data["result"].replace('</br>', '\n')
               hasil = "[ Prank SMS ]\n\n"
               hasil += "Status: "+str(data["status"])+"\n"
               hasil += "Result: "+str(result)+"\n\n[ Finish ]"
